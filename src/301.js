...
this.dartman.ohone = (function() {
	...
	function init() {
		...
		resetElem.addEventListener("mousedown", resetGameClick);
	...
	function runOffRun(e, oops){
		var who = oops || e.target;
		who.volume = 1;
	...
	function doScore(e) {
		...
		if (mode == "add") {
			addScore(theNum);
			soundTockIndex++;
			soundTockIndex = soundTockIndex % soundTock.length;
			soundTock[soundTockIndex].play();
		} else {
			subScore(theNum);
			soundSubIndex++;
			soundSubIndex = soundSubIndex % soundSub.length;
			console.log("soundSubIndex", soundSubIndex);
